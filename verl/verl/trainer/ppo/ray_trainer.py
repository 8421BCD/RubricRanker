...
class RayPPOTrainer:
    ...
    def _maybe_log_val_generations(self, inputs, outputs, scores):
        ...
        # === 改动：每次验证使用不同随机种子（与 global_steps 绑定），让每次验证打印的 10 条不同 ===
        # NOTE: 原本固定种子 42 -> 每次验证 shuffle 出来的 10 条始终一样，不利于观察训练后的 rollout 变化。
        # rng = np.random.RandomState(42)
        rng = np.random.RandomState(int(self.global_steps) if self.global_steps is not None else None)
        rng.shuffle(samples)

        # Take first N samples after shuffling
        samples = samples[:generations_to_log]

        # === 改动：同时把这 N 条随机抽样的 rollout 打印到 stdout（即 train_grpo.log），方便直接查阅 ===
        try:
            print(f"\n========== [val rollouts @ step {self.global_steps}] (random {len(samples)} samples) ==========",
                  flush=True)
            for i, sample in enumerate(samples):
                if len(sample) >= 3:
                    inp, out, sc = sample[0], sample[1], sample[2]
                    print(f"\n----- sample {i + 1}/{len(samples)} | score={sc} -----", flush=True)
                    # print(f"[INPUT]\n{inp}", flush=True)
                    print(f"[OUTPUT]:{out}", flush=True)
                else:
                    print(f"\n----- sample {i + 1}/{len(samples)} -----\n{sample}", flush=True)
            print("========== [val rollouts end] ==========\n", flush=True)
        except Exception as e:
            print(f"[warn] failed to print val rollouts to stdout: {e}", flush=True)
    ...
    def _validate(self, merged: bool = False):
        ...
        for test_data in self.val_dataloader:
            ...
            # ====== DEBUG: 打印 prompt 末尾，检查 <think>/</think> 是否符合预期 ======
            # 不修改上面 sample_inputs 用的 input_texts（保持 skip_special_tokens=True，
            # 避免影响下游 _maybe_log_val_generations / dump_generations 的行为），
            # 这里单独再 decode 一份保留 special token 的版本，仅用于打印观察。
            # try:
            #     pad_id = self.tokenizer.pad_token_id
            #     for i, ids in enumerate(input_ids[:3]):  # 只看前3条，避免日志过大
            #         ids_list = ids.tolist() if hasattr(ids, "tolist") else list(ids)
            #         # 去掉左侧 padding，但保留所有 special token（含 <think>/</think>）
            #         ids_no_pad = [t for t in ids_list if t != pad_id]
            #         full_text = self.tokenizer.decode(ids_no_pad, skip_special_tokens=False)
            #         tail = full_text[-300:] if len(full_text) > 300 else full_text
            #         print(
            #             f"[VAL PROMPT DEBUG] step={self.global_steps} idx={i} "
            #             f"len_tokens={len(ids_no_pad)} tail_300chars=>>>{tail}<<<",
            #             flush=True,
            #         )
            # except Exception as _e:
            #     print(f"[VAL PROMPT DEBUG] failed to dump prompt tail: {_e}", flush=True)
            # ====== END DEBUG ======
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        ...
                        # ====== DEBUG: 训练阶段打印 1 条完整 prompt（保留 special token） ======
                        # 用于检查 train rollout 的 prompt 末尾是否带有 <think>/</think>
                        # try:
                        #     train_prompt_ids = gen_batch_output.batch["prompts"]
                        #     pad_id = self.tokenizer.pad_token_id
                        #     ids = train_prompt_ids[0]
                        #     ids_list = ids.tolist() if hasattr(ids, "tolist") else list(ids)
                        #     ids_no_pad = [t for t in ids_list if t != pad_id]
                        #     full_text = self.tokenizer.decode(ids_no_pad, skip_special_tokens=False)
                        #     print(
                        #         f"[TRAIN PROMPT DEBUG] step={self.global_steps} idx=0 "
                        #         f"len_tokens={len(ids_no_pad)} full_prompt=>>>{full_text}<<<",
                        #         flush=True,
                        #     )
                        # except Exception as _e:
                        #     print(f"[TRAIN PROMPT DEBUG] failed to dump train prompt: {_e}", flush=True)
                        # ====== END DEBUG ======
