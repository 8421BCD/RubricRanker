...
from .utils import extract_snippet_with_context, parse_listwise_ranking
...
class MCPMixin:
    ...
    async def _execute_mcp_call_once(
        ...
        """Execute a single MCP tool call without retry (used by browse tools to avoid retry storms)"""
        return await self._execute_mcp_call_inner(tool_name, params)
    ...
    async def _execute_mcp_call(
        ...
        """Execute an MCP tool call with retry, error handling and global concurrency control"""
        return await self._execute_mcp_call_inner(tool_name, params)

    async def _execute_mcp_call_inner(
        self, tool_name: str, params: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Core MCP call logic with error handling and global concurrency control"""
        if MCPMixin._global_semaphore is None:
            MCPMixin._max_concurrent_calls = int(
                os.environ.get(
                    "MCP_MAX_CONCURRENT_CALLS", DEFAULT_MCP_MAX_CONCURRENT_CALLS
...
class MCPSearchTool(MCPMixin, BaseTool, ABC):
    ...
    async def __call__(
        ...
        # print(f'tool_call_info: {tool_call_info}')
...
class MCPBrowseTool(MCPMixin, BaseTool, ABC):
    ...
    # # Override retry for browse tools - retrying timed-out scrapes wastes
    # # concurrency slots and worsens MCP server overload under high parallelism.
    # @retry(
    #     retry=retry_if_exception_type(
    #         (
    #             ConnectionError,
    ...
    async def _execute_mcp_call(
        ...
        """Override to disable retry for browse tools - retrying timed-out scrapes wastes
        concurrency slots and worsens MCP server overload under high parallelism."""
        return await self._execute_mcp_call_once(tool_name, params)
    ...
    def get_mcp_batch_tool_name(self) -> Optional[str]:
        """Return the MCP tool name for batch webpage fetching.
        Override in subclasses that support batch fetching.
        Returns None if batch fetching is not supported (falls back to per-URL calls)."""
        return None

    def get_mcp_batch_params(self, urls: List[str]) -> Dict[str, Any]:
        """Build parameters for batch MCP webpage fetch.
        Override in subclasses that support batch fetching."""
        raise NotImplementedError("Subclass must implement get_mcp_batch_params for batch fetching")
    ...
    async def _fetch_webpages_parallel(
        ...
        If the subclass supports batch fetching (get_mcp_batch_tool_name returns a name),
        uses a single MCP call with all URLs. Otherwise falls back to per-URL MCP calls.
        ...
        batch_tool_name = self.get_mcp_batch_tool_name()

        if batch_tool_name:
            # Batch mode: single MCP call for all URLs
            return await self._fetch_webpages_batch(docs_to_fetch, batch_tool_name)
        else:
            # Fallback: per-URL MCP calls in parallel
            return await self._fetch_webpages_individual(docs_to_fetch, **kwargs)

    async def _fetch_webpages_batch(
        self, docs_to_fetch: List[Document], batch_tool_name: str
    ) -> Dict[str, Dict[str, Any]]:
        """
        Fetch webpages using a single batch MCP call.
        This is more efficient as it uses only 1 MCP connection + 1 semaphore slot
        instead of N connections + N slots.
        """
        urls = [doc.url for doc in docs_to_fetch]
        url_to_doc_id = {doc.url: doc.id for doc in docs_to_fetch}

        try:
            params = self.get_mcp_batch_params(urls)
            batch_results = await self._execute_mcp_call(batch_tool_name, params)

            # Check for MCP-level errors (timeout, connection failure, etc.)
            # _execute_mcp_call returns {"error": ..., "data": []} on failure
            if isinstance(batch_results, dict) and "error" in batch_results:
                logger.warning(
                    f"Batch MCP call returned error: {batch_results['error']}, "
                    f"falling back to individual fetching"
                )
                return await self._fetch_webpages_individual(docs_to_fetch)

            # batch_results should be a list of dicts, each with a "url" field
            # Map results back to doc IDs
            results_list = None
            if isinstance(batch_results, list):
                results_list = batch_results
            elif isinstance(batch_results, dict) and "results" in batch_results:
                # Handle wrapped response format
                results_list = batch_results["results"]
            else:
                logger.warning(
                    f"Unexpected batch result format: {type(batch_results)}, "
                    f"falling back to individual fetching"
                )
                return await self._fetch_webpages_individual(docs_to_fetch)
            ...
            for result in results_list:
                ...
                doc_id = url_to_doc_id.get(result_url)
                if doc_id:
            ...
            logger.warning(
                f"Batch fetch failed: {e}, falling back to individual fetching"
            )
            return await self._fetch_webpages_individual(docs_to_fetch)

    async def _fetch_webpages_individual(
        self, docs_to_fetch: List[Document], **kwargs
    ) -> Dict[str, Dict[str, Any]]:
        """
        Fetch webpages individually with per-URL MCP calls (original behavior).
        Used as fallback when batch fetching is not supported or fails.
        """
...
class SerperBrowseTool(MCPBrowseTool):
    ...
    def get_mcp_batch_tool_name(self) -> Optional[str]:
        return "serper_fetch_webpages_batch"
    ...
    def get_mcp_batch_params(self, urls: List[str]) -> Dict[str, Any]:
        """Build parameters for batch Serper webpage fetch API"""
        # Dynamically set max_workers based on URL count (cap at 50)
        max_workers = min(len(urls), 50)
        return {"webpage_urls": urls, "include_markdown": True, "max_workers": max_workers}
...
class JinaBrowseTool(MCPBrowseTool):
    ...
    def get_mcp_batch_tool_name(self) -> Optional[str]:
        return "jina_fetch_webpages_batch"
    ...
    def get_mcp_batch_params(self, urls: List[str]) -> Dict[str, Any]:
        """Build parameters for batch Jina webpage fetch API"""
        # Dynamically set max_workers based on URL count (cap at 50)
        max_workers = min(len(urls), 50)
        return {
            "webpage_urls": urls,
    ...
    def _extract_raw_content_from_response(
        ...
        # Prefer fields in this order so that either the Tencent Jina proxy JSON
        # ({"content": ...}) or a raw markdown fallback ({"markdown": ...} /
        # {"text": ...}) is handled transparently.
        content = (
...
class MCPRerankerTool(MCPMixin, BaseTool, ABC):
    ...
    def _listwise_reorder_documents(
        self,
        raw_ranking: str,
        original_documents: List[Document],
    ) -> List[Document]:
        """Shared helper for listwise rerankers.

        Parses the LLM ranking output (e.g. ``"[2] > [5] > [1]"`` or a
        ``<answer>...</answer>`` block containing whitespace-separated indices)
        into an ordering of ``original_documents``, applies ``self.top_n``, and
        returns freshly built :class:`Document` objects whose ``score`` fields
        encode the new rank (descending, so that downstream sorting preserves
        the LLM's chosen order).

        Returns an empty list if the LLM output cannot be parsed; callers should
        treat that as an error.
        """
        ordered_indices = parse_listwise_ranking(raw_ranking, len(original_documents))
        if not ordered_indices:
            warnings.warn(f"Could not parse listwise ranking output: {raw_ranking!r}")
            return []

        # Respect top_n; top_n <= 0 means "keep all".
        effective_top_n = self.top_n if self.top_n > 0 else len(original_documents)
        ordered_indices = ordered_indices[:effective_top_n]

        reranked_documents: List[Document] = []
        for rank, doc_idx in enumerate(ordered_indices):
            original_doc = original_documents[doc_idx]
            reranked_documents.append(
                Document(
                    ...
                    # Descending scores preserve rank order under score-based sorting.
...
class VllmHostedPointwiseRerankerTool(MCPRerankerTool):
    """VLLM-hosted pointwise reranker via Cohere rerank protocol (e.g. BGE)."""
    ...
    def get_mcp_tool_name(self) -> str:
        return "vllm_hosted_pointwise_reranker"
...
VllmHostedRerankerTool = VllmHostedPointwiseRerankerTool


class HFPointwiseRerankerTool(MCPRerankerTool):
    """HF in-process pointwise reranker (monoT5 / RankT5)."""

    def __init__(
        self,
        model_path: str,
        model_kind: str,  # "monot5" | "rankt5"
        device: str = "cuda:0",
        doc_max_words: int = 0,
        batch_size: int = 8,
        max_length: int = 512,
    ...
    def get_mcp_params(
        ...
        return {
            ...
            "device": self.device,
            "doc_max_words": self.doc_max_words,
    ...
    def _process_reranker_results(
        ...
        for r in results:
            if not isinstance(r, dict):
                continue
            idx = int(r.get("index"))
            sc = float(r.get("relevance_score", 0.0))
            if 0 <= idx < len(original_documents):
                d = original_documents[idx]
                reranked.append(
                    Document(
                        id=d.id, title=d.title, snippet=d.snippet,
                        summary=d.summary, url=d.url, text=d.text, score=sc,
                    )
                )
...
class VllmListwiseSlidingWindowRerankerTool(MCPRerankerTool):
    """Sliding-window listwise reranker via vLLM raw completions (RankVicuna / RankZephyr)."""

    def __init__(
        self,
        model_name: str,
        api_url: str,
        model_kind: str = "vicuna",  # "vicuna" | "zephyr"
        window_size: int = 20,
        step: int = 10,
        max_tokens: int = 200,
        doc_max_words: int = 0,
    ...
    def get_mcp_tool_name(self) -> str:
        return "vllm_listwise_sliding_window_reranker"
    ...
    def _process_reranker_results(
        ...
        raw_ranking = raw_output.get("raw_ranking_output", "")
        if not raw_ranking:
            return []
        return self._listwise_reorder_documents(raw_ranking, original_documents)


class _SetwiseRerankerBase(MCPRerankerTool):
    """Base for setwise rerankers (Rank4Gen / SETR): no top_n truncation."""
    ...
    def _parse_indices(self, raw: str, n: int) -> List[int]:
        ...
        out: List[int] = []
        seen = set()
        for m in self._INDEX_RE.findall(raw):
            try:
                idx = int(m) - 1
            except ValueError:
                continue
    ...
    def _process_reranker_results(
        ...
        raw = raw_output.get("raw_ranking_output", "")
        ordered = self._parse_indices(raw, len(original_documents))
        if not ordered:
            return []
        out: List[Document] = []
        for rank, i in enumerate(ordered):
            d = original_documents[i]
            out.append(
                Document(
                    id=d.id, title=d.title, snippet=d.snippet,
                    summary=d.summary, url=d.url, text=d.text,
                    score=1.0 - rank * 1e-3,
                )
            )
        return out
...
class VllmSetwiseRank4GenRerankerTool(_SetwiseRerankerBase):
    """Setwise reranker for Rank4Gen. No top_n truncation."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.downstream_desc = downstream_desc
        self.max_tokens = max_tokens
        self.doc_max_words = doc_max_words

    def get_mcp_tool_name(self) -> str:
        return "vllm_setwise_rank4gen_reranker"
    ...
    def get_mcp_params(
        ...
        return {
            ...
            "downstream_desc": self.downstream_desc,
            "max_tokens": self.max_tokens,
            "doc_max_words": self.doc_max_words,
            "timeout": self.timeout,
        }
...
class VllmSetwiseSETRRerankerTool(_SetwiseRerankerBase):
    """Setwise reranker for SETR. No top_n truncation. Parses '### Final Selection: [..] [..]'."""

    _FINAL_SEL_RE = re.compile(r"###\s*Final\s*Selection:\s*(.*)", re.IGNORECASE | re.DOTALL)
    ...
    def get_mcp_tool_name(self) -> str:
        return "vllm_setwise_setr_reranker"
    ...
    def _parse_indices(self, raw: str, n: int) -> List[int]:
        if not raw:
            return []
        m = self._FINAL_SEL_RE.search(raw)
        tail = m.group(1) if m else raw
        return super()._parse_indices(tail, n)


class VllmSetwiseRubricRankerTool(_SetwiseRerankerBase):
    """Setwise reranker for the student RubricRanker model.

    Behavior mirrors :class:`VllmSetwiseRank4GenRerankerTool`/
    :class:`VllmSetwiseSETRRerankerTool` (no top_n truncation; output is
    parsed as ``[idx] [idx] ...``), but supports two prompt variants:

      - ``"deepresearch"``: includes the agent's reasoning as
        ``query_intent``; pulled from upstream ``DocumentToolOutput.raw_output``
        the same way :class:`OpenAIListwiseRerankerTool` does.
      - ``"rag"``: plain user question, no search_intent.
    """
    ...
    def get_mcp_tool_name(self) -> str:
        return "vllm_setwise_rubricranker_reranker"

    def _extract_search_intent_from_input(
        self, tool_input: DocumentToolOutput
    ) -> str:
        if tool_input.raw_output and isinstance(tool_input.raw_output, dict):
            val = tool_input.raw_output.get("search_intent")
    ...
    def get_mcp_params(
        ...
        return {
            ...
            "model_name": self.model_name,
            "api_url": self.api_url,
            "search_intent": getattr(self, "_current_search_intent", "") or "",
    ...
    async def __call__(
        ...
        """Stash search_intent on self so get_mcp_params can pick it up,
        same trick as OpenAIListwiseRerankerTool.
        """
        if isinstance(tool_input, DocumentToolOutput):
            self._current_search_intent = self._extract_search_intent_from_input(
                tool_input
            )
        else:
            self._current_search_intent = ""
        try:
...
class VllmHostedListwiseRerankerTool(MCPRerankerTool):
    """Tool for reranking documents using a listwise LLM reranker hosted on vLLM via MCP.

    The LLM receives all documents and outputs a ranked list like "[2] > [5] > [1] > ...".
    Parsing of the ranking output is done client-side.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            ...
            top_n=top_n,
            name=name,
            score_threshold=None,  # Listwise reranker doesn't produce individual scores
            **kwargs,
        )
        self.model_name = model_name
        self.api_url = api_url
    ...
    def get_mcp_tool_name(self) -> str:
        return "vllm_hosted_listwise_reranker"
    ...
    def _process_reranker_results(
        ...
        """Parse the listwise LLM output and return reordered documents.

        Delegates to :meth:`MCPRerankerTool._listwise_reorder_documents`, which is
        shared with :class:`OpenAIListwiseRerankerTool`.
        """
        raw_ranking = raw_output.get("raw_ranking_output", "")
        if not raw_ranking:
            return []
...
class VllmListwiseSinglePassRerankerTool(MCPRerankerTool):
    """Single-pass listwise reranker (RankQwen-style) via vLLM chat-completions.

    Uses the RankZephyr training prompt (system + user via chat template,
    no sliding window). Output is "[2] > [5] > [1] > ..." and is parsed
    client-side by :meth:`MCPRerankerTool._listwise_reorder_documents`,
    then truncated to ``top_n`` (just like the hosted listwise tool).
    """
    ...
    def get_mcp_tool_name(self) -> str:
        return "vllm_listwise_singlepass_reranker"
