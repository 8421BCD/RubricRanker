...
def load_dataset(config: DatasetConfig) -> List[Dict]:
    ...
    if config["name"] == "browsecomp":
        ...
        if local_path and Path(local_path).exists():
            # Load custom dataset from local JSONL file
            ...
            raise ValueError(
                ...
                f"Or provide a valid --local-path to a JSONL file."
            )
...
def load_sqav2_data(
    ...
    """Load SQA v2 dataset data from a local JSON file (manually downloaded from HF).

    The original source (``allenai/asta-bench``) is a gated dataset on HuggingFace,
    so we load the file directly from disk to avoid auth issues.
    """
    # Local path to the manually downloaded rubrics file.
    # This file lives at <repo_root>/agent/evaluation/sqa_eval/rubrics_v2_recomputed.json
    # and this source file is at    <repo_root>/agent/dr_agent/dataset_utils/load_dataset.py
    # so we go up two levels from __file__ to reach <repo_root>/agent/.
    local_sqa_path = (
    ...
    if local_sqa_path.exists():
        with open(local_sqa_path, "r") as f:
            data = json.load(f)
    else:
        # Fallback to the original remote loading (requires HF auth)
        print(
            f"Warning: local SQA file not found at {local_sqa_path}, "
            f"falling back to datasets.load_dataset (requires HF auth)."
        )
